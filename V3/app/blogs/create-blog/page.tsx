...
export default function CreateBlogPage() {
  ...
  const addTag = () => {
    // Allows comma-separated tag pasting (e.g., "crypto, web3, airdrop")
    ...
    if (newTags.length > 0) {
      const uniqueTags = Array.from(new Set([...form.tags, ...newTags])).slice(0, 8); // Max 8 tags
      set("tags", uniqueTags);
      setTagInput("");
    }
  };
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5 sm:p-6 space-y-5 backdrop-blur-sm">
            ...
            <div>
              ...
              <p className="text-white/30 text-xs mt-1">
                {form.content.length} chars • ~{Math.max(1, Math.ceil(form.content.split(/\s+/).length / 200))} min read
              </p>
            </div>
