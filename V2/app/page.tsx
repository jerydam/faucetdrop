      ...
      >
        ...
        <section>
          <div className="flex justify-between items-center mb-6 px-2">
            ...
            <Link href="#" className="flex items-center gap-1 text-muted-foreground text-xs sm:text-sm hover:text-foreground">View All <ChevronRight size={14} /></Link>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-x-12 bg-accent/20 rounded-2xl sm:rounded-[2rem] border border-border p-4 sm:p-8">
            {HOT_SPACES.map((s, idx) => (
              <div key={idx} className="flex items-center gap-4 py-3 px-4 hover:bg-accent/50 rounded-xl transition-colors cursor-pointer group">
        ...
        <section>
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="text-muted-foreground hover:text-foreground cursor-pointer"><ArrowRight size={20} /></div>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
            {NEW_SPACES.map((space) => (
              <div key={space.id} className="min-w-[300px] sm:min-w-[320px] bg-card border border-border rounded-2xl p-6 relative group hover:border-primary/50 transition-all duration-300">
                <div className="absolute top-4 right-4 bg-primary text-primary-foreground text-[10px] font-black px-2 py-0.5 rounded uppercase shadow-sm">New</div>
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {space.tags.map(tag => (
                        <span key={tag} className="text-[9px] bg-accent border border-border px-1.5 py-0.5 rounded text-muted-foreground">{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-y-4 pt-4 border-t border-border">
                  <div><p className="text-[9px] text-muted-foreground font-bold uppercase mb-1">Quests</p><p className="text-sm font-bold">{space.quests}</p></div>
                  <div><p className="text-[9px] text-muted-foreground font-bold uppercase mb-1">Funding</p><p className="text-xs font-bold truncate">{space.funding}</p></div>
                </div>
              </div>
            ))}
          </div>
