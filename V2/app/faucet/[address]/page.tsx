...
const loadCustomXPostTemplate = async (faucetAddress: string): Promise<string> => {
  ...
  export default function FaucetDetails() {
    ...
    try {
        ...
        // --- FETCH CUSTOM X POST TEMPLATE ---
        const fetchedTemplate = await loadCustomXPostTemplate(faucetAddress);
        
        // 1. Update the Edittable State
        setCustomXPostTemplate(fetchedTemplate);
        
        // 2. Attach it to the details object so AdminView can compare current vs original
        setFaucetDetails({
            ...details,
            customXPostTemplate: fetchedTemplate // This allows: customXPostTemplate !== details.customXPostTemplate
        })
        // -------------------------------------
    ...
    setUserIsWhitelisted, setUserCustomClaimAmount, setHasCustomAmount, setAdminList, setShowAdminPopup
      ...
      // Inside FaucetDetails component
const generateXPostContent = (amount: string): string => {
    // 1. Determine which template to use
    // Use the constant ONLY if the custom template is empty, whitespace, or still the default placeholder
    const isTemplateEmpty = !customXPostTemplate || !customXPostTemplate.trim();
    const isStillDefault = customXPostTemplate === DEFAULT_X_POST_TEMPLATE;

    let finalTemplate = "";

    if (isTemplateEmpty || isStillDefault) {
        finalTemplate = CONSTANT_X_POST;
    } else {
        // Since the admin edited it, merge the prefix with their custom message
        finalTemplate = `${FIXED_TWEET_PREFIX} ${customXPostTemplate}`;
    }

    // 2. Construct the explorer link safely
    ...
    const explorerLink = txHash && baseUrl 
        ? `${baseUrl}/tx/${txHash}` 
    ...
    return finalTemplate
        ...
        // Clean up placeholders if the user didn't fill them
        .replace(/\{@handle\}/g, "") 
        .replace(/\{#hashtag\}/g, "")
        .trim();
};
