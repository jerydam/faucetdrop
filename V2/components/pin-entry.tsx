...
//   const grant = await requestSigningGrant()   // preferred helper below
//   — OR —
...
export function PinEntryModal() {
  ...
  const triggerBiometric = async () => {
    ...
    try {
      ...
      if (!data) {
        // No local passkey data — fall back to PIN entry
        setMode("pin")
        ...
        <div className="p-6">
          ...
          <div className="flex gap-1 mb-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className={`flex-1 h-0.5 rounded-full transition-all duration-100 ${i < pin.length ? "bg-primary" : "bg-border"}`}
              />
            ))}
          </div>
