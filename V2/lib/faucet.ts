...
/**
 * ✅ REFACTORED: getFaucetsForNetwork to support offset/limit pagination.
 * It fetches ALL addresses first (the necessary bottleneck) and then only fetches 
 * details for the addresses required for the current page.
 * This function correctly uses the 'limit' value (which is 6 from the calling component).
 */
...
): Promise<{ faucets: any[]; totalFaucets: number }> {
    try {
        let allFaucetAddresses: string[] = [];
        let totalFaucets = 0;

        // 1. Collect ALL faucet addresses from all factories (This is the slow part)
        for (const factoryAddress of network.factoryAddresses) {
            if (!isAddress(factoryAddress)) continue;
            const code = await provider.getCode(factoryAddress);
            if (code === "0x") continue;
...
            try {
                const addresses: string[] = await factoryContract.getAllFaucets();
                allFaucetAddresses.push(...addresses.filter(addr => addr && addr !== ZeroAddress));
            } catch (error) {
                console.error(`Error calling getAllFaucets for ${factoryAddress}:`, error);
            }
        }
        
        // Use a Set to ensure unique addresses across multiple factories
        const uniqueFaucetAddresses = Array.from(new Set(allFaucetAddresses));
        totalFaucets = uniqueFaucetAddresses.length;
        console.log(`Total unique faucet addresses found: ${totalFaucets}`);

        // 2. Apply Pagination (Offset & Limit)
        const offset = (page - 1) * limit;
        // Sort addresses alphabetically before slicing to ensure consistent pagination ordering
        uniqueFaucetAddresses.sort(); 
        const addressesForPage = uniqueFaucetAddresses.slice(offset, offset + limit);

        if (addressesForPage.length === 0) {
            console.log(`✅ Fetched 0 faucets for page ${page}`);
            return { faucets: [], totalFaucets };
        }

        // 3. Fetch Details for ONLY the paginated subset of faucets (Faster step)
        console.log(`🔄 Fetching details for ${addressesForPage.length} faucets for page ${page}`);
        
        const results = await Promise.all(
            addressesForPage.map(async (faucetAddress: string) => {
                try {
                    // Try to detect or infer faucet type for correct ABI
                    const faucetType = await detectFaucetType(provider, faucetAddress);
                    const faucetConfig = getFaucetConfig(faucetType);
                    const faucetContract = new Contract(faucetAddress, faucetConfig.abi, provider);
...
                        },
                        factoryAddress: network.factoryAddresses[0], // Simplified for display
                        factoryType: faucetType,
                    };
                } catch (error) {
...
        );
        
        const faucets = results.filter((result) => result !== null);
        console.log(`✅ Fetched ${faucets.length} faucets for page ${page}`);

        return { faucets, totalFaucets };
    } catch (error) {
        console.error(`Error fetching faucets for ${network.name}:`, error);
        return { faucets: [], totalFaucets: 0 };
    }
}
