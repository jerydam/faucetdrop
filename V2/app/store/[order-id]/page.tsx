...
import {
  ...
  ArrowRight, Loader2, ShoppingBag,
} from 'lucide-react';
      ...
      <div className="px-6 py-6 space-y-6">
              ...
              return (
                <div key={step.key} className="flex flex-col items-center gap-2 flex-1">
                  ...
                  <div className="text-center px-1">
                    ...
                    {active && (
                      <p className="text-[9px] text-primary mt-0.5 hidden sm:block">{step.desc}</p>
                    )}
                  </div>
                </div>
              );
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-accent
                      border border-border text-xs text-muted-foreground hover:text-foreground
                      transition-colors"
                  >
                    <Copy size={11} /> Copy
                  </button>
                  {order.trackingUrl && (
                    <a href={order.trackingUrl} target="_blank" rel="noreferrer"
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary/10
                        border border-primary/25 text-xs text-primary hover:bg-primary/20
                        transition-colors">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border/50">
          <p className="text-[10px] text-muted-foreground">
            Questions?{" "}
            <a href="mailto:support@faucetdrops.io" className="text-primary hover:underline">
...
function TrackingContent() {
  ...
  // Auto-fetch when arriving from the orders list page
  useEffect(() => {
    if (incomingId) {
    ...
    <main className="max-w-3xl mx-auto px-4 sm:px-6 pt-8 pb-20">
      ...
      <form onSubmit={handleSearch} className="flex gap-2 mb-8">
        ...
        <button
          ...
          className="px-5 py-3.5 bg-primary text-primary-foreground rounded-2xl font-bold text-sm
        ...
        <div className="border border-dashed border-border rounded-3xl p-12
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground max-w-xs leading-relaxed">
              Your order ID is in the confirmation email you received after redeeming your Drop Points.
            </p>
          </div>
          <Link href="/store/orders"
