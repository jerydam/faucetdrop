...
import { BrowserProvider, JsonRpcProvider, formatUnits } from "ethers";
...
function QuizGameOver({
  ...
  const [rewardAmountRaw, setRewardAmountRaw] = useState<string>("");
  ...
  useEffect(() => {
    ...
    const checkContract = async (
      ...
      const rpcUrl = CHAIN_RPC[contractChainId];
      if (!rpcUrl) {
        console.warn("❌ [ClaimCheck] No RPC for chainId:", contractChainId);
        return;
      }
      ...
      try {
        ...
        if (hasReward && rawStr) setRewardAmountRaw(rawStr);
    ...
    const init = async () => {
      try {
        ...
        if (!data.success) { setClaimStatus("not_eligible"); return; }

        const contractAddress = data.quiz?.reward?.contractAddress || data.quiz?.faucetAddress;
        const contractChainId = data.quiz?.chainId;
        const tokenDecimals = data.quiz?.reward?.tokenDecimals ?? 18;
        const tokenSymbol = data.quiz?.reward?.tokenSymbol ?? "";

        if (!contractAddress || !contractChainId) { setClaimStatus("not_eligible"); return; }

        setContractInfo({ address: contractAddress, chainId: contractChainId, tokenDecimals, tokenSymbol });
        await checkContract(contractAddress, contractChainId, tokenDecimals, tokenSymbol);

        intervalId = setInterval(() => {
          if (!cancelled) checkContract(contractAddress, contractChainId, tokenDecimals, tokenSymbol);
        }, 5000);
      } catch (e) {
  ...
  const handleSwitchAndClaim = async () => {
    ...
    // Only switch if we're certain we're on the wrong chain
    if (chainId !== null && chainId !== contractInfo.chainId) {
      try {
        toast.info("Switching to the correct network...");
        await switchChain(contractInfo.chainId);
        await new Promise(r => setTimeout(r, 1000));
      } catch (e: any) {
        // For embedded wallets switchChain never throws, for external it might
        // if already on correct chain — either way, proceed to claim
        console.warn("[Claim] chain switch warning:", e?.message);
      }
    }
  ...
  const handleClaim = async () => {
    if (!userWalletAddress) { toast.error("Wallet not connected"); return; }
    setIsClaiming(true);
    ...
    try {
      ...
      if (!claimedTx) toast.error(e.message || "Failed to process claim");
    } finally {
      setIsClaiming(false);
    }
  };
    ...
    if (claimStatus === "claimed") {
      return (
        <div className="text-right">
          ...
          <Badge className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">✓ Claimed</Badge>
        </div>
      );
    }
    ...
    if (claimStatus === "expired") {
      return (
        <div className="text-right">
          ...
          <Badge className="bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400">⏰ Claim Expired</Badge>
        </div>
      );
    }
    return (
      <Badge className="bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">Not eligible</Badge>
    );
  };
        ...
        {viewingProfile && (
          <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-4" onClick={() => setViewingProfile(null)}>
            ...
            <div className="relative w-full max-w-sm bg-surface-card border border-surface rounded-3xl p-6 shadow-2xl space-y-4" onClick={e => e.stopPropagation()}>
              ...
              <button onClick={() => { router.push(`/dashboard/${viewingProfile.username}`); setViewingProfile(null); }} className="w-full h-11 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-all active:scale-95">
                View Full Profile
              </button>
            </div>
          </div>
          ...
          <div className="text-center space-y-2">
            <div className="relative w-full max-w-md mx-auto aspect-video rounded-2xl overflow-hidden shadow-lg border border-surface mb-4">
              ...
              <div className="absolute bottom-3 left-3 right-3">
                <h1 className="text-white font-black text-lg sm:text-2xl text-left [text-shadow:0_2px_8px_rgba(0,0,0,0.9),0_1px_3px_rgba(0,0,0,1)]">{rQuiz?.title || quizMeta?.title}</h1>
              </div>
            </div>
            <div className="flex items-center justify-center gap-3 flex-wrap text-surface-secondary text-sm">
              ...
              {endedAt && <><span className="text-slate-300 dark:text-slate-700">•</span><span>{formatDate(endedAt)}</span></>}
            </div>
          </div>
          ...
          {myWallet && (() => {
            ...
            return (
              <div className={cn("rounded-2xl p-4 border flex items-center gap-4",
                ...
                <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl shrink-0",
                ...
                <div className="text-right shrink-0 space-y-1.5"><ClaimStatusUI /></div>
              </div>
            );
          })()}
    ...
    <div className="fixed inset-0 bg-surface-base flex flex-col overflow-auto">
      ...
      <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 pt-8 sm:pt-12 pb-24 space-y-6 sm:space-y-8">
        ...
        {myWallet && (() => {
          ...
          if (!myEntry) return null;
          return (
            <div className={cn("max-w-xl mx-auto w-full rounded-2xl p-4 border flex items-center gap-4 shadow-sm",
              myEntry.rank === 1 ? "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-700/30" :
                myEntry.rank <= 3 ? "bg-blue-50 dark:bg-[#072474]/20 border-blue-200 dark:border-[#072474]/30" :
                  "bg-surface-card border border-surface"
            )}>
...
interface FundRewardButtonProps {
  quizReward: { contractAddress: string; tokenSymbol: string; tokenLogoUrl: string; poolAmount: string; } | null;
  isFunded: boolean;
  isFunding: boolean;
...
export function FundRewardButton({ quizReward, isFunded, isFunding, isFundedCheckLoading, contractBalance, fundTxHash, fundError, onFund, chainId }: FundRewardButtonProps) {
  ...
  return (
    <div className="space-y-3">
      {isFunded ? (
        <div className="flex items-center gap-3 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-900 rounded-xl px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-green-600 dark:text-green-500 mt-0.5">{contractBalance} {quizReward.tokenSymbol} locked in contract</p>
          </div>
          {fundTxHash && explorerBase[chainId] && (
            <a href={`${explorerBase[chainId]}${fundTxHash}`} target="_blank" rel="noopener noreferrer" className="text-green-600 dark:text-green-400 hover:text-green-700 shrink-0">
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-xl px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-amber-700 dark:text-amber-400">Reward Pool Not Yet Funded</p>
              <p className="text-xs text-amber-600 dark:text-amber-500 mt-0.5">
                Fund {quizReward.poolAmount} {quizReward.tokenSymbol} to enable the Start button.
              </p>
              {isFundedCheckLoading && <p className="text-[10px] text-amber-500 mt-1 animate-pulse">Checking balance…</p>}
            </div>
          </div>
          <button onClick={onFund} disabled={isFunding}
            className={["w-full h-12 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all",
              isFunding ? "bg-blue-400 dark:bg-[#072474] text-white cursor-wait" : "bg-[#072474] hover:bg-[#0a32a0] active:bg-[#05184d] text-white shadow-md shadow-[#072474]/20",
            ].join(" ")}>
            {isFunding ? (
              <><span className="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Confirm in wallet…</>
            ) : (
              <><Wallet className="h-4 w-4" />Fund {quizReward.poolAmount} {quizReward.tokenSymbol}</>
            )}
          </button>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 px-1">
        ...
        <p className="text-[10px] font-mono text-surface-secondary truncate flex-1">{quizReward.contractAddress}</p>
      </div>
    </div>
  );
}
...
function FloatingChat({ messages, chatInput, setChatInput, onSend, onSendPreset, myWallet, chatBottomRef, playerCount }: {
  ...
  const PRESET_MESSAGES = ["👋 Hey everyone!", "🔥 Let's go!", "😤 I'm ready!", "🍀 Good luck all!", "😎 Easy win", "🤔 Any hints?"];
  ...
  return (
    <>
      ...
      <div className={cn("fixed bottom-0 right-0 z-50 flex flex-col transition-all duration-300 ease-out",
        "w-full sm:w-[420px]", "sm:bottom-6 sm:right-6 sm:rounded-2xl sm:shadow-2xl",
                  ...
                  <div key={`${m.wallet}-${m.timestamp}-${i}`} className={cn("flex items-end gap-2", isMe && "flex-row-reverse")}>
                    <div className={cn("shrink-0 mb-0.5", !showMeta && "invisible")}>
                      <Avatar className="h-7 w-7">
                        ...
                        <AvatarFallback className="text-[9px] font-bold bg-slate-700 text-white">{m.username?.slice(0, 2).toUpperCase()}</AvatarFallback>
                      </Avatar>
                    </div>
          ...
          <div className="px-3 pt-2 border-t border-surface shrink-0">
            <div className="flex gap-1.5 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
              {PRESET_MESSAGES.map(preset => (
                <button key={preset} onClick={() => onSendPreset(preset)}
...
export default function QuizCodePage() {
  ...
  useEffect(() => {
    ...
    const checkFunded = async () => {
      ...
      try {
        // Use a direct RPC provider for read-only balance checks
        const rpcUrl = CHAIN_RPC[quizReward.chainId ?? chainId ?? 0];
        if (!rpcUrl) return;

        const readProvider = new BrowserProvider(
        ...
        if (!cancelled) {
          ...
          if (result.isFunded) {
            ...
            fetch(`${API_BASE_URL}/api/quiz/${code}/mark-funded`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ txHash: "auto-detected-on-reload", contractAddress: quizReward.contractAddress }),
    ...
    return () => { cancelled = true; clearInterval(intervalId); };
  }, [isCreator, quizReward?.contractAddress, chainId, provider, code]);
  ...
  const handleToggleReady = () => {
    ...
    setPlayers(prev => prev.map(p =>
      p.walletAddress.toLowerCase() === myWallet.toLowerCase() ? { ...p, isReady: newState } : p
    ));
    wsRef.current?.send(JSON.stringify({ type: "set_ready", walletAddress: userWalletAddress, isReady: newState }));
  };
  ...
  const connectWS = useCallback(() => {
    ...
    ws.onmessage = async (ev) => {
      ...
      switch (msg.type) {
        ...
        case "waiting_for_ready": { toast.warning(msg.message || "Some players are not ready yet!"); break; }
        case "kicked": { toast.error(msg.message || "You were removed from this quiz."); router.push("/quiz"); break; }
        case "countdown": { setPhase("countdown"); setCountdownVal(msg.value); break; }
        case "chat_history": {
          seenMessageIds.current.clear();
          const messages = msg.messages as ChatMessage[];
          messages.forEach(m => { seenMessageIds.current.add(`${m.wallet}-${m.timestamp}-${m.text}`); });
        ...
        case "question": {
          ...
          setCurrentQ({ index: msg.index, total: msg.total, question: msg.question, options: msg.options, timeLimit: msg.timeLimit, startedAt: localStartTime });
          setSelectedId(null);
        ...
        case "rewards_dispatched": { toast.success("🏆 Winners have been whitelisted! Claim window is now open."); setRewardsReady(true); break; }
        case "error": { console.error("[WS ERROR]", msg.message); toast.error(msg.message || "Something went wrong"); setIsStarting(false); break; }
        default: console.log("[WS] Unhandled message type:", msg.type, msg);
      }
    };
    ...
    ws.onclose = (event) => {
      ...
      if (reconnectAttempts.current >= 5) { toast.error("Lost connection to the quiz server. Please refresh the page."); return; }
      reconnectAttempts.current += 1;
      const delay = 2000 * reconnectAttempts.current;
      setTimeout(() => { if (wsRef.current?.readyState !== WebSocket.OPEN) connectWS(); }, delay);
    };
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (d.success) {
        ...
        if (d.status === "active") toast.success("Joined mid-game! Wait for the next question.");
        else toast.success(d.message || "Joined quiz!");
      } else if (d.finished) {
        setPhase("game_over");
  ...
  const handleFundReward = async () => {
    ...
    if (!chainId) { toast.error("Wallet not connected"); return; }
    setIsFunding(true);
    setFundError("");

    try {
      const signer = await getActiveSigner(quizReward.chainId ?? chainId);
      if (!signer) throw new Error("Could not get signer");

      const signerProvider = signer.provider;
      if (!signerProvider) throw new Error("No provider on signer");

      const { txHash } = await fundQuizReward(
        signerProvider as any,
        quizReward.chainId ?? chainId,
        quizReward.contractAddress,
        {
  ...
  const handleSelectAnswer = (optId: string) => {
    ...
    if (!hasSubmittedOnChain.current && userWalletAddress) {
      hasSubmittedOnChain.current = true;
    }
  };
        ...
        <div className="space-y-3">
          {!username ? (
            ...
            <>
              ...
              <Button className="w-full h-14 text-lg font-bold bg-[#072474] hover:bg-[#0a32a0] active:bg-[#05184d] text-white rounded-2xl shadow-xl shadow-[#072474]/20 border-0 transition-all active:scale-95" onClick={handleJoin} disabled={isJoining}>
  ...
  if (phase === "lobby") {
    ...
    return (
      <div className="min-h-screen bg-surface-base flex flex-col">
        <div className="sticky top-0 z-20 bg-surface-header backdrop-blur-md border-b border-surface shadow-sm">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Button variant="outline" size="sm" className="border-surface text-surface-secondary hover:text-surface-primary hover:bg-surface-card-2 bg-transparent h-8 px-3" onClick={() => setShowShareModal(true)}>
                      ...
                      {players.map(p => {
                        ...
                        return (
                          <div key={p.walletAddress} className={cn("relative flex flex-col items-center gap-2 rounded-2xl p-3 border text-center transition-all",
                            isHost ? "border-yellow-500/30 bg-yellow-500/10" : ready ? "border-green-500/20 bg-green-500/8" : "border-surface bg-white/3"
                          )}>
                            {isCreator && !isMe && !isHost && (
                              <button onClick={() => handleKickPlayer(p.walletAddress)} className="absolute top-2 right-2 w-5 h-5 rounded-full flex items-center justify-center text-red-400 hover:text-red-300 hover:bg-red-500/20 transition-all">
                                <X className="h-3 w-3" />
                              </button>
                            )}
                            <div className="relative">
                              <Avatar className="h-12 w-12 border-2 border-surface">
                                ...
                                <AvatarFallback className={cn("font-bold text-sm", isHost ? "bg-yellow-500/20 text-yellow-300" : "bg-white/10 text-white")}>
                            ...
                            <div className="min-w-0 w-full">
                              ...
                              <p className={cn("text-[10px] font-semibold mt-0.5", isHost ? "text-yellow-400" : ready ? "text-green-400" : "text-surface-muted")}>
                ...
                <div className="bg-surface-card border border-surface rounded-2xl overflow-hidden">
                  ...
                  <div className="p-4 space-y-3">
                    <div className={cn("rounded-xl px-4 py-3 flex items-center gap-3 border",
                      allReady ? "bg-green-500/8 border-green-500/20" : totalCount === 0 ? "bg-white/3 border-white/8" : "bg-amber-500/8 border-amber-500/20"
                    )}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={cn("text-sm font-bold", allReady ? "text-green-400" : totalCount === 0 ? "text-surface-muted" : "text-amber-400")}>
                    ...
                    {quizReward ? (
                        ...
                        <div className="space-y-2">
                          <div className="flex items-start gap-3 bg-amber-500/8 border border-amber-500/20 rounded-xl px-4 py-3">
                            ...
                            <div>
                              ...
                              <p className="text-amber-500/70 text-xs mt-0.5">The quiz cannot start until the reward pool is funded</p>
                            </div>
                          </div>
                          <Button className="w-full h-11 font-bold bg-[#072474] hover:bg-[#0a32a0] text-white border-0 text-sm" onClick={handleFundReward} disabled={isFunding || isFundedCheckLoading}>
                            {isFunding ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Confirming...</> : <><Wallet className="mr-2 h-4 w-4" />Fund {grossDisplayAmount} {quizReward.tokenSymbol}</>}
                          </Button>
                        </div>
                      )
                    ) : (
                      <div className="flex items-start gap-3 bg-amber-500/8 border border-amber-500/20 rounded-xl px-4 py-3">
                        ...
                        <div>
                          ...
                          <p className="text-amber-500/70 text-xs mt-0.5">A funded reward pool is required to start this quiz</p>
                        </div>
                      </div>
                    )}

                    {isFunded && (
                      <Button className="w-full h-14 text-base font-black text-white border-0 rounded-xl shadow-lg shadow-[#072474]/40 disabled:opacity-50 transition-all hover:scale-[1.02] active:scale-[0.98] bg-[#072474] hover:bg-[#0a32a0]" onClick={handleStartQuiz} disabled={isStarting}>
                    ...
                    {fundError && <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2 break-words">{fundError}</p>}
                    {quizReward && <p className="text-[10px] font-mono text-surface-muted truncate text-center">{quizReward.contractAddress}</p>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-surface-card border border-surface rounded-2xl overflow-hidden">
                <div className="px-5 py-3 border-b border-surface flex items-center justify-between">
                  <span className="text-surface-primary font-bold text-sm flex items-center gap-2"><Users className="h-4 w-4 text-blue-400" /> Players ({players.length})</span>
                  {totalCount > 0 && (
                  ...
                  {players.map(p => {
                    ...
                    return (
                      <div key={p.walletAddress} className="flex flex-col items-center gap-1.5 text-center">
                        <div className="relative">
                          <Avatar className={cn("h-12 w-12 border-2", isMe ? "border-blue-400" : isHost ? "border-yellow-400" : ready ? "border-green-400" : "border-surface")}>
                            <AvatarImage src={p.avatarUrl ?? undefined} />
              ...
              {hasJoined && (
                <div className="max-w-sm mx-auto">
                  <button onClick={handleToggleReady} className={cn("w-full h-14 rounded-2xl font-black text-base flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg",
                    isReady ? "bg-green-500 hover:bg-green-400 text-white shadow-green-900/30" : "bg-[#072474] hover:bg-[#0a32a0] text-white shadow-[#072474]/40"
                  )}>
                    {isReady ? <><Check className="h-5 w-5 stroke-[3px]" /> You're Ready! (tap to undo)</> : <><Zap className="h-5 w-5 fill-current" /> Click to Ready Up</>}
                  </button>
                  {isReady && (
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowShareModal(false)}>
            <div className="bg-surface-card border border-surface rounded-3xl p-6 sm:p-8 shadow-2xl max-w-sm w-full text-center space-y-5" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowShareModal(false)} className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-surface-muted hover:text-surface-primary transition-colors">
                  <X className="h-5 w-5" />
                </button>
              </div>
                ...
                const downloadQR = () => {
                  ...
                  img.onload = () => {
                    ...
                    canvas.toBlob(pngBlob => {
                      ...
                      a.href = pngUrl; a.download = `quiz-${code}-qr.png`; a.click();
                  ...
                  <div className="space-y-3">
                    <div className="relative group mx-auto w-max">
                      <div className="bg-white p-4 rounded-2xl shadow-inner">
                        <QRCodeSVG id="share-qr-svg" value={quizUrl} size={180} level="H" includeMargin={true} />
                      </div>
                      <button onClick={downloadQR} className="absolute inset-0 rounded-2xl bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                        <div className="bg-white text-slate-900 rounded-xl px-4 py-2 font-bold text-sm flex items-center gap-2 shadow-lg">
                          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M12 15V3m0 12l-4-4m4 4l4-4M2 17l.621 2.485A2 2 0 004.561 21h14.878a2 2 0 001.94-1.515L22 17" strokeLinecap="round" strokeLinejoin="round" /></svg>
                          Download PNG
                        </div>
                      </button>
                    </div>
                    <button onClick={downloadQR} className="w-full h-10 rounded-xl border border-surface bg-surface-card-2 text-surface-secondary hover:text-surface-primary hover:bg-surface-card text-sm font-bold flex items-center justify-center gap-2 transition-all active:scale-95 sm:hidden">
                      <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M12 15V3m0 12l-4-4m4 4l4-4M2 17l.621 2.485A2 2 0 004.561 21h14.878a2 2 0 001.94-1.515L22 17" strokeLinecap="round" strokeLinejoin="round" /></svg>
                      Download QR Code
                    </button>
                  </div>
              ...
              <div className="space-y-1.5">
                <p className="text-surface-secondary text-xs font-bold uppercase tracking-widest">Quiz Code</p>
                <div className="text-4xl font-black tracking-widest text-[#072474] dark:text-blue-400 bg-[#072474]/5 dark:bg-[#072474]/20 py-3 rounded-xl border border-[#072474]/20">{code}</div>
              </div>

              <Button className="w-full h-12 font-bold bg-[#072474] hover:bg-[#0a32a0] text-white rounded-xl shadow-md border-0 transition-all active:scale-95"
                onClick={() => {
        ...
        {hasJoined && (
          <FloatingChat messages={chatMessages} chatInput={chatInput} setChatInput={setChatInput}
          ...
          {currentQ.options.map(opt => {
            ...
            return (
              <button key={opt.id} disabled={isSpectator || isReveal || timeLeft <= 0} onClick={() => handleSelectAnswer(opt.id)}
                className={cn("relative w-full flex items-center justify-between px-6 py-5 sm:py-6 md:py-8 rounded-2xl last:mb-4",
                  "text-surface-primary font-bold text-lg md:text-xl transition-all duration-150",
                  "active:scale-[0.98] cursor-pointer shadow-md", style.bg,
