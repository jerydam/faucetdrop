  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col p-4 sm:p-5 space-y-3">
        {post.tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {post.tags.slice(0, 3).map(tag => (
              <span key={tag} className="text-[10px] h-4 px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 font-semibold">
                {tag}
              </span>
            ))}
          </div>
        ...
        {post.excerpt && (
          <p className="text-white/50 text-sm line-clamp-2 flex-1">{post.excerpt}</p>
        )}

        <div className="mt-auto pt-3 border-t border-white/10 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <div className="h-6 w-6 rounded-full bg-indigo-500/30 border border-indigo-500/40 overflow-hidden shrink-0 flex items-center justify-center">
              {post.author_avatar
                ? <img src={post.author_avatar} alt="" className="w-full h-full object-cover" />
                : <span className="text-[10px] font-bold text-indigo-300">{post.author_name?.slice(0, 2).toUpperCase() || "FD"}</span>
              }
            </div>
        ...
        <div className="p-5 sm:p-8 flex flex-col justify-between space-y-4">
          <div className="space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="bg-indigo-500 text-white border-0 text-xs px-2.5 py-0.5 rounded-full font-bold">
                Latest
              </span>
              {post.tags.slice(0, 2).map(tag => (
                <span key={tag} className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 font-semibold">
                  {tag}
                </span>
              ))}
            </div>
            <h2 className="text-xl sm:text-2xl font-black text-white leading-tight group-hover:text-indigo-300 transition-colors">
              {post.title}
            </h2>
            {post.excerpt && (
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <div className="h-9 w-9 rounded-full bg-indigo-500/30 border border-indigo-500/40 overflow-hidden flex items-center justify-center shrink-0">
                {post.author_avatar
                  ? <img src={post.author_avatar} alt="" className="w-full h-full object-cover" />
                  : <span className="text-xs font-bold text-indigo-300">{post.author_name?.slice(0, 2).toUpperCase() || "FD"}</span>
                }
              </div>
              <div>
...
export default function BlogPage() {
  ...
  useEffect(() => { fetchPosts(1, activeTag); }, [activeTag]);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8 pb-20">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-4 pt-4">
          ...
          <p className="text-white/50 text-base sm:text-lg">
            The latest from the FaucetDrops team — product updates, announcements and insights.
          </p>
          <button
            onClick={() => fetchPosts(1, activeTag)}
            disabled={loading}
            ...
            <button
              ...
              className={cn(
                ...
                !activeTag
                  ...
                  : "bg-white/5 text-white/50 border-white/10 hover:border-indigo-500/40 hover:text-white/80"
              )}
            >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-24 text-center">
            ...
            <button
              onClick={() => fetchPosts()}
              className="px-5 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white text-sm font-bold hover:bg-white/15 transition-all"
            >
              Retry
            </button>
          </div>
          ...
          <div className="space-y-6">
            ...
            {page < totalPages && (
              <div className="flex justify-center pt-4">
                <button
                  onClick={() => fetchPosts(page + 1, activeTag, true)}
                  disabled={loadingMore}
                  className="flex items-center gap-2 h-11 px-8 rounded-xl bg-white/5 border border-white/10 text-white text-sm font-bold hover:bg-white/10 hover:border-white/20 transition-all disabled:opacity-50"
                >
                  {loadingMore
