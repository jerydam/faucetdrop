...
export default function CreateSinglePage() {
  ...
  const { address, session, chainId } = useWallet();

  const chainCfg = useMemo(() => (chainId ? getChainConfig(chainId) : null), [chainId]);
  ...
  try {
    ...
    const reserveRes = await fetch(`${API_BASE_URL}/api/challenge/single/reserve`, {
      ...
      body: JSON.stringify({ creatorAddress: address, chainId: chainId ?? 42220 }),
    })
    ...
    const activeSigner = await getActiveSigner(chainId ?? 42220)
    ...
    const chainCfg = getChainConfig(chainId ?? 42220)
