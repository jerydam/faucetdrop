...
export function NetworkProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    // ── Solana connected ────────────────────────────────────────────────────
    // Solana doesn't emit an EVM chainId. When the Solana adapter is connected
    // we manually set the Solana Devnet network entry.
    if (solanaConnected && solanaPublicKey) {
      const solanaNetwork = networks.find((n) => n.chainId === SOLANA_CHAIN_ID)
      if (solanaNetwork && network?.chainId !== SOLANA_CHAIN_ID) {
        console.log("[NetworkProvider] ✅ Solana wallet connected — setting Solana Devnet")
    ...
    if (!evmAddress) {
      setIsConnecting(false)
      // Only clear if we were on Solana and disconnected
      if (network?.chainId === SOLANA_CHAIN_ID) setNetworkState(null)
      return
    }

    // ── EVM wallet: awaiting chainId ────────────────────────────────────────
    ...
    if (matched) {
      if (network?.chainId !== currentChainId) {
        console.log(`[NetworkProvider] ✅ EVM network: ${matched.name}`)
      ...
      console.warn(`[NetworkProvider] ⚠️ Unsupported chainId: ${currentChainId}`)
      setNetworkState(null)
      toast({
        title: "Unsupported Network",
        description: `Chain ID ${currentChainId} is not supported. Please switch to Celo, Lisk, Arbitrum, Base, or BNB.`,
        variant: "destructive",
      })
    }
  }, [rawEvmChainId, evmAddress, currentChainId, solanaConnected, solanaPublicKey])
  // NOTE: intentionally omitting `network` and `toast` from deps to avoid
  // infinite loops — they're read only for side-effects here.
