...
function FaucetCard({ faucet }) {
    return (<Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-lg flex items-center justify-between">
          <span>{faucet.tokenSymbol || "Token"} Faucet</span>
          {faucet.isClaimActive ? (<span className="text-xs bg-green-500/20 text-green-600 dark:text-green-400 px-2 py-0.5 rounded-full">
              Active
            </span>) : (<span className="text-xs bg-red-500/20 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-full">
              Inactive
            </span>)}
        </CardTitle>
...
export function FaucetList() {
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {faucets.map((faucet) => (<FaucetCard key={faucet.faucetAddress} faucet={faucet}/>))}
    </div>);
}
